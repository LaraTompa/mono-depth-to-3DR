...
def svd_orthogonalize(M: torch.Tensor) -> torch.Tensor:
    ...
    """
    orig_dtype = M.dtype
    # Disable AMP autocast entirely for this block. Otherwise, the ATen 
    # dispatcher intercepts the @ operator and silently downcasts float32
    # tensors back to fp16 before passing them to the next function.
    ...
    return R.to(orig_dtype)                                  # (B, 3, 3) ∈ SO(3)
